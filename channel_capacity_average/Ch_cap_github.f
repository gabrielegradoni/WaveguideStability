             ...
             Real*8 LE, LE_old
c-------------------------------
c          Channel  capacity  versus the corrugation number eps in [0,0.5]
c          averaged on phase space.   Output Chan_cap.dat
c             
c          Input parameters: iteration number N_max , grid points number N_grid
c-------------------------------              

             open(10,file='Ch_cap.dat')
             pi=4*datan(1.d0)
             duepi=2*pi
c---            
             N_max=200        
...
c--            
             ...
             do I_eps= 1,  98  ! --------------------->   Main loop eps
  
             eps=0.005*I_eps
             write(*,'(I5,g13.5)') I_eps,eps
             ...
             do Ix=1,N_grid      !  --------------->  Grid loop 
                x0= pi*(-1+  2*Ix/A_grid) !   -pi < x < pi
         
             do Iv=1,   N_grid   !  ---------->  Grid loop              
                 v0=-0.9999 + 1.9998 * Iv/A_grid ! -1 < v <1
...
c-----
c           Phase space averages on the grid of Channel Capacities                 
c----                        
             Ch_LE= Ch_LE+ dlog(LE)/(N_max*N_grid**2) ! N_grid**2)         
             Ch_RE= Ch_RE+ dlog(RE)/(N_max*N_grid**2) ! N_grid**2)             
             Ch_REM=Ch_RE+ dlog(REM)/(N_max*N_grid**2)! N_grid**2)         

             enddo     !  end of the x_0 loop
             enddo     !  end of the v_0 loop

            
             write(10,'(5g13.5)') eps,Ch_LE,Ch_RE,Ch_REM          
             enddo ! <--------------------
             return
             end
